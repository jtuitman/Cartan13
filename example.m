...
for i:=1 to #G1_list do
  if G1_list[i] ne 0 then
    PhiAZ1b[i]:=parallel_transport(teichpoints[i],Qppoints[i],Z1,eta1,data:prec:=100)*frob_equiv_iso(G1_list[i],data);
  else
    PhiAZ1b[i]:=0;
  end if;
end for;
...
for i:=1 to #G2_list do
  if G2_list[i] ne 0 then
    PhiAZ2b[i]:=parallel_transport(teichpoints[i],Qppoints[i],Z2,eta2,data:prec:=100)*frob_equiv_iso(G2_list[i],data);
  else
    PhiAZ2b[i]:=0;
  end if;
end for;
...
K<alpha> := UnramifiedExtension(Qp, CharacteristicPolynomial(Aq_small));     // End(J) \otimes QQ
